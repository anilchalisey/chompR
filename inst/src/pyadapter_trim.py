...
if append == "fastq":
    p1_rds = open(p1_in,'r')
    p2_rds = open(p2_in,'r')
    p1_out = re.sub(".fastq", ".trim.fastq", p1_in)
    p2_out = re.sub(".fastq", ".trim.fastq", p2_in)
elif append == "fq":
    p1_rds = open(p1_in,'r')
    p2_rds = open(p2_in,'r')
    p1_out = re.sub(".fq", ".trim.fastq", p1_in)
    p2_out = re.sub(".fq", ".trim.fastq", p2_in)
elif append == "gz":
    p1_rds = gzip.open(p1_in,'r')
    p2_rds = gzip.open(p2_in,'r')
    p1_out = re.sub(".fastq.gz", ".trim.fastq", p1_in)
    p2_out = re.sub(".fastq.gz", ".trim.fastq", p2_in)
elif append == "bz2":
    p1_rds = bz2.BZ2File(p1_in,'r')
    p2_rds = bz2.BZ2File(p2_in,'r')
    p1_out = re.sub(".fastq.bz2", ".trim.fastq", p1_in)
    p2_out = re.sub(".fastq.bz2", ".trim.fastq", p2_in)
else:
